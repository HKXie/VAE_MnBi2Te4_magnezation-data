...
def generate_random_image(batch_size,size):
    random_data = torch.rand(batch_size,size)
    return random_data
...
#VAEmodel，Encoder and Decoder 
class VAE(nn.Module):
    def __init__(self, h_dim=400, z_dim=20,learning_rate=1e-4, class_num=50):
        super().__init__()
        ...
        self.convlinear = nn.Linear(32*6*6 + class_num, h_dim)
        self.convtralinear = nn.Linear(h_dim, 32*6*6)
        ...
        self.convtraseq = nn.Sequential(
           
            nn.ConvTranspose2d(32, 64, kernel_size=3, stride=1),#2*10*10
            # nn.BatchNorm2d(64),
            nn.LeakyReLU(),
            # nn.GELU(),
    ...
    def encode(self, image, label_tensor):
        # h = F.relu(self.fc1(x))
        # output = self.fc1(x)
        # output = self.act1(output)
        output = self.convseq(image)
        output = self.act1(output)

        output = output.view(-1,32*6*6)

        output = torch.cat((output, label_tensor), axis=1)
    ...
    def decode(self, z, label_tensor):
        ...
        output = self.fc4(z)
        output = self.act1(output)
        output = self.convtralinear(output)
        output = output.view(-1,32,6,6)
        output = self.convtraseq(output)
        
        output = output.view(-1,2,10,10)
    ...
    # def loss_function(self, x_reconst, x, mu, log_var,batch_size): #lossfunction, reconstructed loss + KL loss 

    #     Loss = nn.BCELoss(reduction='sum')#
    #     # Loss = nn.MSELoss(reduction='sum')#

    #     # Loss = nn.CrossEntropyLoss(reduction='sum')#效果不行
    ...
    #     train_loss, reconstruction_loss, kl_loss = self.loss_function(gen_imgs, real_imgs, mu, log_varbatch_size)
